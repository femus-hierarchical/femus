...
MultiGrid::MultiGrid(const char mesh_file[], const unsigned short& gridP, 
		     FunctionBoundary BoundaryFunctionOther, FunctionInit InitFunctionOther, FunctionlocalP localPFunctionOther): // 0-based index used throughout the class.
  gridr(gridP),
  _BoundaryFunction (BoundaryFunctionOther),
  _InitFunction(InitFunctionOther),
  _localPFunction(localPFunctionOther)  
  {
  time_test = 0;
  int gaussOrder = 18;
  p = gridP;
...
// 	}
	...
	//cout<<"prolongate, restrict: "<<static_cast<double>(prolongate)/CLOCKS_PER_SEC<<"s"<<endl;
        ...
        L2Error(); //Compute L2 error
/*
 Print solution on hierarchical vtk file
 */
  printVtkHierarchical(p);
/*
 Read hierarchical file and invoke visualization library's functions
 */
  //hVisualize vtkFile("../mesh-generator/output/hierarchical.2.vtk");
	for(int i=0;i<p;i++)
	  Print_vtk_ASCII(0,i);
...
void MultiGrid::Initialize(const char name[]){
  ...
  for(unsigned i=i_start; i<i_end; i++){
    CheckVectorSize(i); 
    for(unsigned j=0; j< D[p-1]->el->NumOfDof(SolType[i]-1u); j++) 
      Sol[i][j]=0.;
  }
}

void MultiGrid::CheckVectorSize(const unsigned& i){
  if(Sol[i].size() < D[p-1]->el->NumOfDof(SolType[i]-1u)) 
...
void MultiGrid::ResizeSolutionVector(const char name[]){
  ...
  for(unsigned i=i_start;i<i_end;i++){
    Sol[i].resize(D[p-1]->el->NumOfDof(SolType[i]-1u)); 
    BC[i].resize(D[p-1]->el->NumOfDof(SolType[i]-1u));	
...
void MultiGrid::GenerateBC(const char name[]){ // grid numbers in 0-based index
  ...
  for(unsigned i=i_start;i<i_end;i++){ //loop on solution vectors
	...
	z += vt[2][vIndex[k]];
      ...
      int localP = _localPFunction(x/numOfVertices,y/numOfVertices,z/numOfVertices,p); 
	...
	if( D[g]->el->FacetAdjacentElemIndex(iel,facet) < 0 ){ // if the facet is on the boundary
	  ...
	  for(unsigned k=0; k<nNodes; k++){
	    ...
	    if( iNode<D[g]->MeshNodeNum(0) ){ //if the the node is a vertex 
	      test = _BoundaryFunction(vt[0][iNode],vt[1][iNode],vt[2][iNode],SolName[i],value,-(D[g]->el->FacetAdjacentElemIndex(iel,facet)+1));
    ...
    }//cout<<"#dofs to solve: "<<DofstoSolve[p-1]<<endl;
  }
}
...
void  MultiGrid::Print_vtk_ASCII(const unsigned& time, const int type){ //type is 0-based
  ...
  sprintf(filename,"./output/mesh.%d.%d.vtk",time,type);
  std::ofstream fout;
  fout.open(filename);
  if (!fout) {
...
void  MultiGrid::printVtkHierarchical(const int type)
{
  char* filename= new char[60];
  sprintf(filename,"./output/hierarchical.%d.vtk",type);
  std::ofstream fout;
  fout.open(filename);
  if (!fout) {
    cout << "Output mesh file "<<filename<<" cannot be opened.\n";
  ...
  /* header */
  fout<<"# vtk p-hierarchical DataFile Version 3.0\nAMR mesh\nASCII\n";
  
  /* Geometry / Topology */
  fout<<"DATASET UNSTRUCTURED_GRID\n"; 
  
  PetscInt nvt = D[p-1]->MeshNodeNum(0);
  fout<<"VERTICES "<<nvt<<" double\n";
  
  for(PetscInt i=0;i<nvt;i++)
    fout<<vt[0][i]<<" "<<vt[1][i]<<" "<<vt[2][i]<<endl;
  
  PetscInt nel = D[p-1]->MeshElementNum();
  
  PetscInt counter = D[p-1]->el->MeshElementNum("Hex")*9 + D[p-1]->el->MeshElementNum("Tet")*5 + D[p-1]->el->MeshElementNum("Wedge")*7
  + D[p-1]->el->MeshElementNum("Quad")*5 + D[p-1]->el->MeshElementNum("Triangle")*4 + D[p-1]->el->MeshElementNum("Line")*3;
  
  fout<<"ELEMENTS "<<nel<<" "<<counter<<endl; // counter: total number of integers required to represent the list of cells. 
      
  fout<<"ELEMENT_TYPES "<<nel<<endl;
  for(unsigned j=0; j<nel; j++)
     fout<<D[p-1]->el->ElementType(j)<<endl;
  
  fout<<"POLYNOMIAL_ORDERS "<<nel<<endl;
  // Local polynomial order
  vector<short> polyOrder;
  polyOrder.resize(nel);

  double x,y,z;
  for(unsigned i=0; i<nel; i++){
    // find centroid
      unsigned numOfVertices=D[p-1]->el->ElementNodeNum(i,0);
      int vIndex[numOfVertices];
      x=0.;y=0.;z=0.;
      
      for(unsigned k=0;k<numOfVertices;k++){
	vIndex[k] = D[p-1]->el->GlobalNodeIndex(i,k)-1u;	
	x += vt[0][vIndex[k]];
	y += vt[1][vIndex[k]];
	z += vt[2][vIndex[k]];
      }
      
      polyOrder[i] = _localPFunction(x/numOfVertices,y/numOfVertices,z/numOfVertices,p); 
      fout<<polyOrder[i]-1<<endl;
  }
  
  fout<<"GLOBAL_DOFS "<<nel<<endl;
  for(unsigned i=0; i<nel; i++){    
    PetscInt elemDof = type_elem[D[p-1]->el->ElementType(i)][polyOrder[i]-1]->ElementDOF();
    fout<<elemDof<<" ";
    for(unsigned j=0; j<elemDof; j++)
      fout<<D[p-1]->el->globalDOF_(i)[j]<<" ";
    fout<<endl;
  }
  /* solution */
  fout<<"\nSOLUTION "<<DOF[p-1]<<endl;
  for(unsigned i=0;i<Sol.size();i++){
    fout<<"SCALARS Sol"<<SolName[i]<<" double 1"<<endl<<"LOOKUP_TABLE default\n";
