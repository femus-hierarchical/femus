...
void hVisualize::readHVtk(const char hVtk_file[]){  
  ...
  for(unsigned i=0;i<numberOfElements;i++){
    ...
    for(unsigned j=0;j<elemDof[i];j++)
      fin >> globalDof[i][j];
  }
...
//     SetFacetAdjacentElemIndex(iel,iface,adjElem[fIndex[iface]]);
  ...
  for(unsigned i=0; i<=maxP; i++){
    ...
    pt[i][0] = new hexh(i+1,0);//TODO fix this 1-based 0-based thing
  ...
  for(unsigned iel=0;iel<numberOfElements;iel++){ 
    ...
    sign[iel].resize(pt[p[iel]][elemType[iel]]->dimOfBasis());
    pt[p[iel]][elemType[iel]]->eval_sign(&o[iel][0], sign[iel]);
    
    type[iel].resize(pt[p[iel]][elemType[iel]]->dimOfBasis());
...
void hVisualize::writeVtk()
{
  ...
  std::ofstream fout;
  fout.open(filename);
  if (!fout) { //TODO exception handling
 ...
 void hVisualize::hierarchicalToNodalProjection2D(){ // extend this for multiple variables. LagrangeOrder is 1-based
 ...
 vector < vector <double> > vt; // coordinates of the vertices of each element
  ...
  for(unsigned kel=0; kel < numberOfElements; kel++){
    LElemDof = pt[elemType[kel]-3]->ElementDOF(p[kel]+1); //TODO lagrange order is 1-based that's why. Consider modifyinig lagrange elemDof function to be 0-based
    LSolSize += LElemDof;
  }
...
//   }
  ...
  unsigned QuadElemDof = pt[0]->ElementDOF(maxP+1); //TODO LagrangeOrder 1-based
  ...
  for(unsigned i=0; i < QuadElemDof; i++)
    phi[i] = new double[type_elem[3][maxP]->ElementDOF()]; //size of quad. //TODO type_elem ONLY 2D
  int nVertices[2]={4,3};
  
  for(unsigned kel=0; kel < numberOfElements; kel++){
    
    LElemDof = pt[elemType[kel]-3]->ElementDOF(p[kel]+1);//TODO lagrange order is 1-based that's why.
...
//TODO once LagrangeOrder is 0-based, change inside type_elem too.
    for(unsigned i=0;i<LElemDof;i++){
      LSol[i+offset]=0.; 
...
void hVisualize::hierarchicalToNodalProjection3D(){ // extend this for multiple variables. LagrangeOrder is 1-based
...
//   order.resize(Sol.size());
  ...
  for(unsigned kel=0; kel < numberOfElements; kel++){
    LElemDof = pt[0]->ElementDOF(p[kel]+1); //TODO lagrange order is 1-based that's why. Consider modifyinig lagrange elemDof function to be 0-based
    LSolSize += LElemDof;
  }
...
//   }
  ...
  unsigned HexElemDof = pt[0]->ElementDOF(maxP+1); //TODO LagrangeOrder 1-based
  
  double** phi = new double*[HexElemDof]; // create projection matrix to the size of hex
  
  for(unsigned i=0; i < HexElemDof; i++)
    phi[i] = new double[type_elem[0][maxP]->ElementDOF()]; //size of quad. change this to hex when you do 3-D
  
  for(unsigned kel=0; kel < numberOfElements; kel++){
    LElemDof = pt[0]->ElementDOF(p[kel]+1);//TODO lagrange order is 1-based that's why.
    HElemDof = type_elem[elemType[kel]][p[kel]]->ElementDOF();
...
int hVisualize::pTohProjection2D()
{
  ...
  unsigned CellSize = numberOfElements*pow((maxP+1),2);
  ...
  pt[0] = &quad_;
  pt[1] = &tri_;
  const unsigned nVertices[2] = {4,3};
  int nQuads=0;
  int nTris=0;
  ...
  for(unsigned kel=0; kel < numberOfElements; kel++){
    ...
    row = 0; drow=0;
    for(unsigned i=0;i<pow((p_l),2);i++){
...
int hVisualize::pTohProjection3D()
{
  ...
  unsigned CellSize = numberOfElements*pow((maxP+1),3);
  Cell.resize(CellSize);
  CellType.resize(CellSize);
  ...
  for(unsigned kel=0; kel < numberOfElements; kel++){
    ...
    for(unsigned i=0;i<pow((p_l),3);i++){
