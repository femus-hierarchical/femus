...
elem_type::~elem_type(){
  ...
  for(int i=0;i<typeSize[solid_];i++){
    for(int j=0;j<numOfGaussPts;j++)
      delete[] phi[i][j];
    delete [] phi[i];
  }
...
elem_type::elem_type(const int solid, const int p, const int n, const int orderOfMapping, int* globalIndex){ //1-based index
  solid_ = solid;
  hierarchicP = p;
  ...
  if(solid_==0) pt_h[1] = new hexh(p,1);
  if(solid_==1) pt_h[1] = new teth(p,1);
  if(solid_==2) {
    for(int i=1;i<6;i++)
  ...
  for(int i=0; i<p-1;i++){ // p: 1-based
    ...
    if((i>0)&&(solid_ == 4)){ //additional tri interior mode (if element=triangle)
      IND[count][0]=2;
      IND[count][1]=0;
      IND[count][2]=interior;
      count++; interior++;
  ...
  if(i>0){//additional tri face mode
    ...
    if((i>1)&&(solid_==1)) // internal modes for tet
    ...
    else if((i>2)&&(solid_==2)) // internal modes for wedge
    ...
    else if((i>3)&&(solid_==0)) // internal modes for hex
  ...
  phi = new double**[typeSize[solid_]];
  dphidxi = new double**[typeSize[solid_]];
  dphideta = new double**[typeSize[solid_]];
  dphidzeta = new double**[typeSize[solid_]];
  ...
  for(int k=0;k<typeSize[solid_];k++)
    for(int i=0;i<numOfGaussPts;i++)
  ...
  SetOrientationFlagsAndType(solid, globalIndex);
  SetSign(o_(), type_()); // Set sign and type for each basis function
...
void elem_type::ProjectionMatrix2D(const int p_, vector <vector <double> > vertex, double** P, vector <vector <double> > & node) const{
...
void elem_type::ProjectionMatrix3D(const int p_, vector <vector <double> > vertex, double** P, vector <vector <double> > & node) const{
...
void elem_type::mapSpatialToRefElem1D(vector<vector<double> >node, const unsigned& gaussIndex, double& weight, double* phi_, double** gradPhi, double* X) const{
  double det;
  double J = 0;
...
void elem_type::mapSpatialToRefElem2D(vector<vector<double> > node, const unsigned& gaussIndex, double& weight, double* phi_, double** gradPhi, double* x) const{
  ...
  for(int i=0;i<numOfShapeFuns;i++){
   ...
   if(solid_==4){ // correct for linear map only. Do for general case.
    x[0]=0.;x[1]=0;x[2]=0.;
    for(int coordinate=0;coordinate<3;coordinate++)
      for(int k=0;k<3;k++)
...
void elem_type::mapSpatialToRefElem3D(vector<vector<double> > node, const unsigned& gaussIndex, double& weight, double* phi_, double** gradPhi, double* x) const{
...
// Evaluates the sign and type based on orientation flags / type Do this one inside elem_type
void elem_type::SetSign(const int* o, const int* referenceElementType){
    
    sign.resize(numOfShapeFuns);
    pt_h[0]->eval_sign(&o[0], sign);
    
    type.resize(numOfShapeFuns);
    pt_h[0]->eval_type(&referenceElementType[0], type);
