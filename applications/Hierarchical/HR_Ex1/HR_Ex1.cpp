...
unsigned localP(const double& x, const double& y, const double& z, unsigned& P);
...
int main(int argc, char **argv){
  ...
  try{
    ...
    std::stringstream s;
    s << "./input/mesh.mesh_" << DIMENSION + 1  <<  "D_hex2";//;_square10by10;_tri10by10_hybrid_finalHybrid
...
//     fout<<"p \t DOF \t L2 error";
    ...
    for(int i=p-1;i<p;i++){
      start_time = clock();
      mg = new MultiGrid(infile, i+1, BoundaryND, Init, localP);
      delete mg;
      end_time = clock();
...
unsigned localP(const double& x, const double& y, const double& z, unsigned& P){// centroid of the element
  int localp = P;
  if(x<0.5) localp = P-1;
    return localp;
}
